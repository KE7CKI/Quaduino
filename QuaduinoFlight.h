...
private:
	...
	int minPulseWidth; // = 1100; // minimum pulse width in microseconds
	int maxPulseWidth; // = 1600; // maximum pulse width in microseconds
	...
	// though this is probably a bit low
	// (added to min_pulse_width, typically resulting in ~1300)
	float gravityHover;// = 200;

	// fine-tuning motor offset values to accomodate fixed imbalances
	// or differences in each motor, which there will be some
	float frontMotorOffset; // = 11;
	float rearMotorOffset; //  = 5;
	float rightMotorOffset; // = 7;
	float leftMotorOffset; // = 9;
	...
	PID * rollPID;//(&rollInput, &rollOutput, &rollSetpoint, consKp, consKi, consKd, DIRECT);
        ...
        enum Motor { FRONT, RIGHT, REAR, LEFT };
public:
	QuaduinoFlight();
	...
	void setupPidControl();
        ...
        // =============================================================
        void writeMotor(uint8_t m, uint8_t val);
	...
	// =============================================================
	void adjustPitch(float pitchAngle);

	// =============================================================
	// ADJUST ROLL USING PID/STEERING
	// =============================================================
	void adjustRoll(float rollAngle);

	// =============================================================
	// ADJUST YAW USING PID/STEERING (NOT IMPLEMENTED YET)
	// =============================================================
	void adjustYaw(float yawAngle);
